...
class TextToNumbers:
    ...
    def convert(self, text_line):
        ...
        parsed_list, result_text_list = self.russian.parse(text=text_line)
        converted_text = ""
        parsed_idx = 0
                ...
                for idx, val in enumerate(converted_text):
                    if val == "т":
                        if converted_text[idx + 4] == "а" \
                                and converted_text[idx - 2] in numbers \
                                and converted_text[idx + 6] in numbers \
                ...
                for idx, val in enumerate(converted_text):
                    if val == "з":
                        if converted_text[idx + 6] == "я" \
                                and converted_text[idx - 2] in numbers \
                                and converted_text[idx + 8] in numbers \
